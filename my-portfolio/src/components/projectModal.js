...
export default function ProjectModal({ imageUrl, title, description, tags, githubUrl, liveUrl }) {
    useAutoBoldText();

    return (
        <div style={styles.container} className="project-modal-container">
            <div style={styles.imageContainer} className="project-modal-image-container">
                <img src={imageUrl} alt={title} style={styles.image} />
            </div>
            <div style={styles.contentContainer} className="project-modal-content-container">
                <h2 style={styles.title}>🚀 {title}</h2>
                <p style={styles.description} className="auto-format">{description}</p>
                <div style={styles.tagContainer}>
                    {tags.map((tag, index) => (
                        <span className="tag" key={index} style={styles.tag}>{tag}</span>
                    ))}
                </div>
                <div style={styles.buttonContainer} className="project-modal-button-container">

                    <a href={githubUrl} target="_blank">
                        <button style={styles.button}> <svg style={{
                            top: '2px',
                            position: 'relative',
                            scale: '1.1'
